...
int initBoard(FILE *fp)
{
	...
	for (i = 0; i< maxY; ++i)
	{
		...
		puts(board[i]);  /* 읽어들인 배열 출력 */ // 문자열찍고 자동개행
	}
...
int boggleGame(const char *searchWord)
{
	...
	} //바깥for문이 행제어, 안쪽for문이 열제어 5행5열이면 총 25번
	return FALSE;
}
/*-------------------------------------------------------------------
...
int hasWord(int y, int x, const char * word)
{
	...
	if (inRange(y, x) == FALSE) return FALSE;  /* basis part1 : 시작 위치가 범위 밖이면 실패 return FALSE;*/

	if (board[y][x] != *word) return FALSE;     /* basis part2 : word문자열의 첫 글자가 x, y 좌표의 내용과 일치하지 않으면 실패return FALSE; */
			
	if (strlen(word) == 1) return TRUE;    /* basis part3 : 단어의 길이가 1이면 성공 <== basis part2보다 반드시 나중에 검사해야 함 return TRUE;*/
	
	nextY = y;
	nextX = x;
	for (i = 0; i < 8; i++)   /* 8방향에 대해 이어지는 스펠링이 존재하는가 검사 - 재귀 호출이 일어나는 부분*/ //단일for문 (보낼때 주소에 1번지씩늘려서)
	{
